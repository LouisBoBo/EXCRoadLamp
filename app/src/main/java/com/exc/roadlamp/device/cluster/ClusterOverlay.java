...
public class ClusterOverlay implements AMap.OnCameraChangeListener,
    ...
    private void addSingleClusterToMap(Cluster cluster) {
        ...
        markerOptions.anchor(0.5f, 0.5f)
                .icon(getBitmapDes(mClusterItems,cluster.getClusterCount())).position(latlng);
     ...
     * 获取每个聚合点的绘制样式
     */
    private BitmapDescriptor getBitmapDes(List<ClusterItem> clusterItems,int num) {
        ...
        if (bitmapDescriptor == null) {
            ...
            if (mClusterRender != null && mClusterRender.getDrawAble(mClusterItems,num) != null) {
                textView.setBackground(mClusterRender.getDrawAble(mClusterItems,num));
            } else {
    ...
    private void updateCluster(Cluster cluster) {
            ...
            marker.setIcon(getBitmapDes(mClusterItems,cluster.getClusterCount()));
