 ...
 */
public class MyCorePageManager {
    /**
     * 页面信息
     */
    private static final String PAGE_INFO_JSON = "corepage.json";

    private volatile static MyCorePageManager sInstance = null;
    /**
     * 保存page的map
     */
     ...
     */
    public static MyCorePageManager getInstance() {
        if (sInstance == null) {
            synchronized (MyCorePageManager.class) {
                if (sInstance == null) {
                    sInstance = new MyCorePageManager();
                }
            }
        }
        return sInstance;
     ...
     * @return 是否新增成功
     */
    public boolean putPage(String name, Class<? extends MyXPageFragment> clazz, Map<String, String> params) {
     ...
     */
    public MyXPageFragment openPageWithNewFragmentManager(FragmentManager fragmentManager, String pageName, Bundle bundle, int[] animations, boolean addToBackStack) {
     ...
     * @return 打开的Fragment对象
     */
    public MyXPageFragment changePageWithNewFragmentManager(FragmentManager fragmentManager, String pageName, Bundle bundle, int[] animations, boolean addToBackStack) {
        MyXPageFragment xPageFragment = null;
        try {
            CorePage corePage = mPageMap.get(pageName);
            if (corePage == null) {
                PageLog.d("Page:" + pageName + " is null");
            ...
            xPageFragment = (MyXPageFragment) fragmentManager.findFragmentByTag(pageName);
     ...
     */
    private MyXPageFragment loadXPageFragmentByCorePage(CorePage corePage, String pageName, Bundle bundle) throws ClassNotFoundException, IllegalAccessException, InstantiationException {
        MyXPageFragment fragment;

        //====Atlas的支持 start====/
        if (MyCoreConfig.isOpenAtlas()) {
            ClassLoader bundleClassLoader = MyCoreConfig.getBundleClassLoader();
            if (bundleClassLoader == null) {
                PageLog.d("OpenAtlas bundle ClassLoader is null!");
                return null;
            }
            fragment = (MyXPageFragment) MyCoreConfig.getBundleClassLoader().loadClass(corePage.getClazz()).newInstance();
        } else {
            fragment = (MyXPageFragment) Class.forName(corePage.getClazz()).newInstance();
        }
        //====Atlas的支持 end====/
    ...
    public boolean isFragmentTop(Context context, String fragmentTag) {
        if (context != null && context instanceof MyCoreSwitcher) {
            return ((MyCoreSwitcher) context).isFragmentTop(fragmentTag);
        } else {
